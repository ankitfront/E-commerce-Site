...
const Add = ({token}) => {
  ...
  const submitHandeler = async(e)=>{
    ...
    try {
      ...
      formData.append("price", Number(price)); // ✅ FIXED
      ...
      console.log("ADD PRODUCT ERROR:", error.response?.data || error.message); // ✅ FIXED
      toast.error(error.message);
    }
  }

  return (
    <form onSubmit={submitHandeler} className="flex flex-col w-full items-center gap-3">
      {/* Upload Image */}
      <div>
      ...
      <button className="px-6 py-2 bg-black text-white mt-4">
        ADD PRODUCT
      </button>
    </form>
  );
